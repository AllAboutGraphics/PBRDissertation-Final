...
void UploadAttribute(GLuint* id, int numElements, int dataSize, int attribSize, int attribID, void* pointer, const string& debugName) {
	...
	/*if (debugName == "Colours")
	{
		...
		float randNum = (float)(rand() % 255) / 255.0f;
		Vector4 data = Vector4(1.0f, 1.0f, randNum, 1.0f);
		memcpy(ptr, &data, 4 * sizeof(float));
...
Mesh* Mesh::GenerateTriangle(float x, float y, float dist)
{
	Mesh* m = new Mesh();
	m->numVertices = 3;
	...
	m->vertices[0] = Vector3(x, y, 0.0f);
	m->vertices[1] = Vector3(x + dist, y - dist, 0.0f);
	m->vertices[2] = Vector3(x - dist, y - dist, 0.0f);
...
Mesh* Mesh::GenerateQuad(float topLeftX, float topLeftY, Vector4 colours, float distance)
{
	Mesh* m = new Mesh();
	m->numVertices = 4;
	m->type = GL_TRIANGLE_STRIP;
	m->vertices = new Vector3[m->numVertices];
	m->vertices[0] = Vector3(topLeftX, topLeftY, 0.0f);										//-1,  1
	m->vertices[1] = Vector3(topLeftX, topLeftY - (2 * distance), 0.0f);					//-1, -1
	m->vertices[2] = Vector3(topLeftX + (2 * distance), topLeftY, 0.0f);					//1,   1
	m->vertices[3] = Vector3(topLeftX + (2 * distance), topLeftY - (2 * distance), 0.0f);	//1,  -1
...
Mesh* Mesh::GenerateCubeForCubeMap()
{
	Mesh* m			 = new Mesh();
	m->type			 = GL_TRIANGLES;
	m->numVertices	 = 36;
	...
	for (int i = 0; i < m->numVertices; i++)
	{
		...
		m->vertices[i]	    = Vector3(vertices[baseIndex], vertices[baseIndex + 1], vertices[baseIndex + 2]);
		m->normals[i]	    = Vector3(vertices[baseIndex + normalOffset], vertices[baseIndex + normalOffset + 1], vertices[baseIndex + normalOffset + 2]);
		m->textureCoords[i] = Vector2(vertices[baseIndex + texCoordOffset], vertices[baseIndex + texCoordOffset + 1]);
	}
	m->BufferData();
	return m;
}
...
//	m->numVertices				  = (X_SEGMENTS + 1) * (Y_SEGMENTS + 1);
...
//
//			m->vertices[(x * X_SEGMENTS) + y]	   = Vector3(xPos, yPos, zPos);
//			m->textureCoords[(x * X_SEGMENTS) + y] = Vector2(xSegment, ySegment);
//			m->normals[(x * X_SEGMENTS) + y]	   = Vector3(xPos, yPos, zPos);
//		}
//	}
//
//	bool oddRow = false;
...
//		oddRow = !oddRow;
//	}
//	m->numIndices = static_cast<unsigned int>(indices.size());
...
Mesh* Mesh::CreateSphere()
{
	...
	for (int i = 0; i < m->numVertices; i++)
	{
		m->vertices[i]		= positions[i];
		m->textureCoords[i] = uv[i];
...
void Mesh::GenerateNormals()
{
	...
	for (int i = 0; i < numVertices; ++i) { normals[i] = Vector3(); }
	...
	for (int i = 0; i < numVertices; ++i) { normals[i].Normalise(); }
}
...
Vector4 Mesh::GenerateTangent(int a, int b, int c)
{
	...
	Vector3 biCross = Vector3::Cross(tangent, normal);	//Calculated to check the handedness of the binormal.
...
void Mesh::GenerateTangents()
{
	...
	for (unsigned int i = 0; i < numVertices; ++i) { tangents[i] = Vector4(0, 0, 0, 0); }
	int triCount = GetTriCount();
	for (int i = 0; i < triCount; ++i)
	{
		unsigned int a = 0;
