...
public:
	...
	static Mesh* GenerateTriangle(float x = 0.0f, float y = 0.5f, float dist = 0.5);	//Top Vertex X, Y coordinates
	static Mesh* GenerateQuad(float topLeftX = -1.0f, float topLeftY = 1.0f, Vector4 colours = Vector4(1.0f, 1.0f, 1.0f, 1.0f), float distance = 1.0f);	//STarts with top left by default according to default inputs. Counter clockwise Winding
	static Mesh* GenerateCubeForCubeMap();
